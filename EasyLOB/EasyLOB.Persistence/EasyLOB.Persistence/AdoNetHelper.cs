...
namespace EasyLOB.Persistence
{
    ...
    public static partial class AdoNetHelper
    {
        ...
        /// <returns></returns>
        public static string SqlGetIsolationLevel(ZDBMS dbms, IsolationLevel isolationLevel)
        {
            string result = "";
        ...
        /// <param name="connectionName">Connection name</param>
        /// <param name="isolationLevel">Isolation level</param>
        public static void SqlSetIsolationLevel(string connectionName, IsolationLevel isolationLevel)
        {
            ...
            try
            {
                ...
                ZDBMS dbms = GetDBMS(connection);
                string sql = SqlGetIsolationLevel(dbms, isolationLevel);
        ...
        /// <summary>
        /// Set DBCommand SQL isolation level.
        /// </summary>
        /// <param name="command">Command</param>
        /// <param name="isolationLevel">Isolation level</param>
        public static void SqlSetIsolationLevel(DbCommand command, IsolationLevel isolationLevel)
        {
            ZDBMS dbms = AdoNetHelper.GetDBMS(command.Connection);
            string sql = AdoNetHelper.SqlGetIsolationLevel(dbms, isolationLevel);
            command.CommandText = (string.IsNullOrEmpty(sql) ? "" : sql + Environment.NewLine)
                + command.CommandText;
        }
        ...
        public static void SqlParameters(DbCommand command)
        {
            DbProviderFactory provider = DbProviderFactories.GetFactory(command.Connection);
        ...
        /// </summary>
        /// <param name="command">Database command</param>
        public static void SqlRecords(DbCommand command)
        {
            SqlRecords(command, RecordsBySearch);
        }
        ...
        public static void SqlRecords(DbCommand command, int? records)
        {
            DbProviderFactory provider = DbProviderFactories.GetFactory(command.Connection);
        ...
        /// <returns></returns>
        public static int SqlCommand(string connectionName, string sql) // , params object[] parameters)
        {
        ...
        public static IEnumerable<dynamic> SqlQueryDynamic(string connectionName, string sql) // , params object[] parameters)
        {
            // <param name="parameters">Parameters</param>
            ...
            using (DbConnection connection = provider.CreateConnection())
            {
                ...
                {
                    List<string> names = Enumerable.Range(0, dataReader.FieldCount).Select(dataReader.GetName).ToList();
                    foreach (IDataRecord row in dataReader as IEnumerable)
                    {
                        var expando = new ExpandoObject() as IDictionary<string, object>;
                        foreach (string name in names)
                        {
                            expando[name] = row[name];
                        }
                        //for (var fieldCount = 0; fieldCount < dataReader.FieldCount; fieldCount++)
                        //{
                        //    expando.Add(dataReader.GetName(fieldCount), dataReader[fieldCount]);
                        //}
